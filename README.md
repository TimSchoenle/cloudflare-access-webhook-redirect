...
`just` with no arguments lists the recipes. `just verify` is the fmt, clippy and test a pull
request runs anyway. After changing anything under `src/config/`, run `just regenerate`: it
