...
The last two are generated by `cargo run --example config-schema`, which walks the `Config`
type — so the settings tables below cannot drift from the keys the service actually reads. The
first two are what keeps the pinned-image example correct across a release: the release pull
request is the commit that changes those numbers, so it arrives with the rendered README
already updated.
-->
<br/>
<p align="center">
